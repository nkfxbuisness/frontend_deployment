  ...
  const getPageDetails = async()=>{
    try {
      ...
      const { data } = await axios.get(
        `https://server-deployment-jet.vercel.app/api/user/getWithdrawlPageData/${user._id}`,
        config
      );
      console.log(data);
      if(data.succcess){
  ...
  const submit = async()=>{
    ...
    try {
      ...
      const { data } = await axios.post(
        `https://server-deployment-jet.vercel.app/api/user/withdrawalRequest/${user._id}`,
        {
          amount
        },
        config
